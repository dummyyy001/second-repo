/*global kadence_wootemplate_params */
;(function ( $, window ) {
	$( '.kadence-status-toggle' ).on( 'click', function( event ) {
		...
		$.ajax( {
			type: 'POST',
			url: kadence_wootemplate_params.ajax_url,
			data: {
				action           : 'kadence_wootemplate_change_status',
				post_id          : $button.data( 'post-id' ),
				post_status      : $button.data( 'post-status' ),
				security         : kadence_wootemplate_params.ajax_nonce
			},
			dataType: 'json',
			success: function( response ) {
				...
				if ( response && response.success ) {
					if ( 'publish' === $button.data( 'post-status' ) ) {
						...
						$button.find( '.kadence-status-label' ).html( kadence_wootemplate_params.draft );
					} else {
						$button.removeClass( 'kadence-status-draft' );
						$button.addClass( 'kadence-status-publish' );
						$button.data( 'post-status', 'publish' );
						$button.find( '.kadence-status-label' ).html( kadence_wootemplate_params.publish );
					}
					$button.closest( 'tr.type-kadence_wootemplate' ).find( '.column-title .post-state' ).hide();
				} else {
