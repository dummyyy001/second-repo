	...
	class Kadence_Plugin_API_Manager {
		...
		public function api_ithemes_field( $args ) {
			...
			if ( isset( $data['api_email'] ) && ! empty( $data['api_email'] ) ) {
				...
				if ( $args['p_key'] === 'kadence_gutenberg_pro' || $args['p_key'] === 'kadence_woo' || $args['p_key'] === 'kadence_cloud' || $args['p_key'] === 'kadence_conversions' ) {
					if ( ! empty( $data['api_key'] ) || ! empty( $data['ithemes_key'] ) ) {
		...
		public function activate_ithemes( $args ) {
			if ( 'kadence_gutenberg_pro' !== $args['product_id'] && 'kadence_woo' !== $args['product_id'] && 'kadence_cloud' !== $args['product_id'] && 'kadence_conversions' !== $args['product_id'] ) {
				// Not setup yet.
				return false;
			}
			if ( 'kadence_cloud' === $args['product_id'] ) {
				$product = 'kadence-cloud';
			} elseif ( 'kadence_conversions' === $args['product_id'] ) {
				$product = 'kadence-conversions';
...
$kt_plugin_api->add_product( 'kadence_build_child_defaults_activation', 'kt_api_manager_kadence_build_child_defaults_data', 'kadence_build_child_defaults', 'Kadence Child Theme Builder' );
