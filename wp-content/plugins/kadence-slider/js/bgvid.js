...
!(function(root, factory) {
  ...
  Vidbg.prototype.init = function() {
    ...
    if (typeof path === 'object') {
      ...
      $video = vidbg.$video = $('<video playsinline>' + sources + '</video>');
    } else {
      $video = vidbg.$video = $('<video playsinline>' +
    ...
    .one('canplaythrough.' + PLUGIN_NAME, function() {
      vidbg.resize();
      if (navigator.userAgent.indexOf('Safari') != -1 && navigator.userAgent.indexOf('Chrome') == -1 && ! isMobile_kt_slider.any()) {
    		$video.css({
	        visibility: 'visible',
	        opacity: 1
	      	});
