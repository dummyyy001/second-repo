...
class Request {
    ...
    // If marker param exist
		...
		//	Modify the Stores Load query
		$query  = apply_filters( 'asl_filter_load_stores', $query);
			...
			if($aRow->custom) {
				...
				if($custom_fields && is_array($custom_fields) && count($custom_fields) > 0) {
					...
					foreach ($custom_fields as $custom_key => $custom_value) {
						...
						$aRow->$custom_key = esc_attr($custom_value);
					}
				}
			}
	...
	private function add_test_stores() {
		...
		$file_ = '/home/dev/projects/wordpress_language/locator-data.json';
