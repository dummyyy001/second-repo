...
use WDFQVendorFree\Monolog\Handler\StreamHandler;
use Psr\Log\LogLevel;
use WDFQVendorFree\WPDesk\Logger\WC\WooCommerceCapture;
use WDFQVendorFree\WPDesk\Logger\WP\WPCapture;
/**
 * Manages and facilitates creation of logger
 *
 * @package WPDesk\Logger
 */
class WPDeskLoggerFactory extends \WDFQVendorFree\WPDesk\Logger\BasicLoggerFactory
{
    ...
    public static function tearDown($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (\WDFQVendorFree\Monolog\Registry::hasLogger($name)) {
            \WDFQVendorFree\Monolog\Registry::removeLogger($name);
        }
    }
    /**
    ...
    public function disableLog($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        if (\WDFQVendorFree\Monolog\Registry::hasLogger($name)) {
            /** @var Logger $logger */
            $logger = \WDFQVendorFree\Monolog\Registry::getInstance($name);
    ...
    public function createWPDeskLogger($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (!self::$shouldLoggerBeActivated) {
            return new \WDFQVendorFree\Monolog\Logger($name);
        }
        if (\WDFQVendorFree\Monolog\Registry::hasLogger($name)) {
    ...
    private function captureWPLog()
    {
        static $wpCapture;
        if (!$wpCapture) {
            $wpCapture = new \WDFQVendorFree\WPDesk\Logger\WP\WPCapture(\basename($this->getFileName()));
     ...
     * @param Logger $logger
     */
    private function captureWooCommerce(\WDFQVendorFree\Monolog\Logger $logger)
    {
        if (!\defined('WDFQVendorFree\\WC_LOG_THRESHOLD')) {
            \define('WDFQVendorFree\\WC_LOG_THRESHOLD', self::LEVEL_WC);
        }
        $wcIntegration = new \WDFQVendorFree\WPDesk\Logger\WC\WooCommerceCapture($logger);
     ...
     */
    private function pushFileHandle($filename, \WDFQVendorFree\Monolog\Logger $logger)
    {
        $logger->pushHandler(new \WDFQVendorFree\Monolog\Handler\StreamHandler($filename, self::LEVEL_WPDESK_FILE));
    }
    /**
    ...
    public function getFileName($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        return \trailingslashit(\untrailingslashit($upload_dir['basedir'])) . \WDFQVendorFree\WPDesk\Logger\WP\WPCapture::LOG_DIR . \DIRECTORY_SEPARATOR . $name . '_debug.log';
    }
    /**
     ...
     * @return void
     */
    private function removeAllHandlers(\WDFQVendorFree\Monolog\Logger $logger)
    {
        try {
            while (\true) {
                $logger->popHandler();
            ...
            $logger->pushHandler(new \WDFQVendorFree\Monolog\Handler\NullHandler());
        }
    }
    /**
    ...
    {
        return \WDFQVendorFree\Monolog\Registry::hasLogger($name);
    }
}
