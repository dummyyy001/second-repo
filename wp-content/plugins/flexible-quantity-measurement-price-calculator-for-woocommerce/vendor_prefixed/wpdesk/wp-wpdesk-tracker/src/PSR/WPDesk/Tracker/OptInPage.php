...
namespace WDFQVendorFree\WPDesk\Tracker;
...
class OptInPage implements \WDFQVendorFree\WPDesk\PluginBuilder\Plugin\Hookable
{
    /**
     * @var string
     */
    ...
    public function __construct($plugin_file, $plugin_slug, $shop = null)
    {
        ...
        $this->shop = $shop ?? new \WDFQVendorFree\WPDesk\Tracker\Shop('');
    }
    ...
    {
        \add_submenu_page('', 'WP Desk Tracker', 'WP Desk Tracker', 'manage_options', 'wpdesk_tracker_' . $this->plugin_slug, [$this, 'output']);
    }
    /** @return void */
    public function output()
    {
        ...
        if (isset($_GET['shop_url'])) {
            $shop = new \WDFQVendorFree\WPDesk\Tracker\Shop(\sanitize_text_field(\wp_unslash($_GET['shop_url'])));
        } else {
            $shop = $this->shop;
        }
        ...
        $renderer = new \WDFQVendorFree\WPDesk\View\Renderer\SimplePhpRenderer(new \WDFQVendorFree\WPDesk\View\Resolver\DirResolver(__DIR__ . '/views'));
    ...
    public function admin_init()
    {
        if (isset($_GET['wpdesk_tracker']) && $_GET['wpdesk_tracker'] === $this->plugin_slug) {
            if (isset($_GET['allow']) && isset($_GET['security']) && \wp_verify_nonce($_GET['security'], $this->plugin_slug)) {
                if ($_GET['allow'] === '1') {
                    $persistence = new \WDFQVendorFree\WPDesk_Tracker_Persistence_Consent();
