 ...
 */
class PluginActionLinks implements \WDFQVendorFree\WPDesk\PluginBuilder\Plugin\Hookable
{
    /**
     * @var string
     */
    ...
    public function append_plugin_action_links($links)
    {
        ...
        $tracker_consent = new \WDFQVendorFree\WPDesk_Tracker_Persistence_Consent();
        $plugin_links = [];
        if (!$tracker_consent->is_active()) {
            ...
            $plugin_links[] = '<a href="' . \esc_url($opt_in_link) . '">' . \esc_html__('Enable usage tracking', 'flexible-quantity-measurement-price-calculator-for-woocommerce') . '</a>';
        } else {
            $opt_in_link = \admin_url('plugins.php?wpdesk_tracker_opt_out_' . $this->plugin_slug . '=1&security=' . \wp_create_nonce($this->plugin_slug));
            $opt_in_link = \add_query_arg('plugin', $this->plugin_slug, $opt_in_link);
            $opt_in_link = \add_query_arg('ctx', 'links', $opt_in_link);
            $plugin_links[] = '<a href="' . \esc_url($opt_in_link) . '">' . \esc_html__('Disable usage tracking', 'flexible-quantity-measurement-price-calculator-for-woocommerce') . '</a>';
        }
