...
if (!\class_exists('WDFQVendorFree\\WPDesk_Tracker_Sender_Logged')) {
    class WPDesk_Tracker_Sender_Logged implements \WPDesk_Tracker_Sender
    {
        ...
        {
            if (\class_exists('WDFQVendorFree\\WPDesk_Logger_Factory')) {
                \WDFQVendorFree\WPDesk_Logger_Factory::log_message('Sender payload: ' . \json_encode($payload), self::LOGGER_SOURCE, \WDFQVendorFree\WPDesk_Logger::DEBUG);
                try {
                    $response = $this->sender->send_payload($payload);
                    \WDFQVendorFree\WPDesk_Logger_Factory::log_message('Sender response: ' . \json_encode($response), self::LOGGER_SOURCE, \WDFQVendorFree\WPDesk_Logger::DEBUG);
                    return $response;
                } catch (\WDFQVendorFree\WPDesk_Tracker_Sender_Exception_WpError $exception) {
                    \WDFQVendorFree\WPDesk_Logger_Factory::log_exception($exception);
                    throw $exception;
                }
            } else {
