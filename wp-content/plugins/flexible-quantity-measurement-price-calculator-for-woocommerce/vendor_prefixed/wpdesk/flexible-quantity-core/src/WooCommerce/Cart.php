...
use WC_Cart;
use WC_Order;
use WDFQVendorFree\WC_Order_item_Product;
use WC_Product;
use WC_Product_Variation;
use WDFQVendorFree\WPDesk\PluginBuilder\Plugin\Hookable;
class Cart implements \WDFQVendorFree\WPDesk\PluginBuilder\Plugin\Hookable
{
    ...
    private const DEFAULT_MEASUREMENT_NEEDED_ROUNDING_PRECISION = 4;
    /**
    ...
    public function hooks()
    {
        ...
        \add_filter('woocommerce_add_cart_item', [$this, 'set_product_prices'], 1, 1);
        ...
        \add_filter('woocommerce_cart_contents_weight', [$this, 'set_cart_contents_weight']);
        ...
        \add_action('woocommerce_before_calculate_totals', [$this, 'calculate_product_weights']);
        \add_action('woocommerce_cart_loaded_from_session', [$this, 'calculate_product_weights']);
    ...
    public function add_to_cart_validation($valid, $product_id, $quantity, $variation_id = '', $variations = [], $cart_item_data = [])
    {
        ...
        if ($valid && \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Product::pricing_calculator_enabled($product)) {
            ...
            foreach ($measurements as $measurement) {
                ...
                $value = \abs((float) \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Measurement::convert_to_float($value));
            ...
            if (isset($cart_item_data['pricing_item_meta_data']) && isset($cart_item_data['pricing_item_meta_data']['_measurement_needed'])) {
                ...
                if (\trim($product_settings['fq']['min_range']) !== '') {
                ...
                if (\trim($product_settings['fq']['max_range']) !== '') {
                ...
                if (\trim($product_settings['fq']['increment']) !== '' && !$this->is_mesurement_value_valid_by_increment((float) $measurements_needed_main, (float) $product_settings['fq']['increment'])) {
            ...
            if (isset($_POST['_measurement_needed'], $_POST['_measurement_needed_unit'])) {
                $measurement_needed_value = \wc_clean(\wp_unslash($_POST['_measurement_needed']));
            ...
            // $measurement_needed = new Measurement( $measurement_needed_value_unit, $measurement_needed_value );
            // if ( null === $settings->get_pricing_rules_price( $measurement_needed ) ) {
            // $message = apply_filters( 'fq_price_calculator_no_price_available_notice_text', __( "No price available for a product with this measurement, please contact the store for assistance.", 'flexible-quantity-measurement-price-calculator-for-woocommerce' ), $product );
            // wc_add_notice( $message, 'error' );
            // $valid = false;
            // }
            // }
    ...
    public function get_cart_item_data($cart_item_data, $product_id, $variation_id) : array
    {
        ...
        if (\WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Product::pricing_calculator_enabled($product)) {
            ...
            if ($measurement_needed_value) {
                ...
                $price = \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Product::calculate_price($_product, $measurement_needed_value, $measurement_needed_value_unit);
            ...
            foreach ($settings->get_calculator_measurements() as $measurement) {
                if (isset($_POST[$measurement->get_name() . '_needed'])) {
                    ...
                    if (\WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Helper::str_starts_with($measurement_needed, '.')) {
     ...
     * @return float
     */
    private function calculate_measurement_needed($product, $variation = null)
    {
        $settings = new \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Settings($product);
        ...
        if (!empty($this->measurements_needed) && \is_array($this->measurements_needed)) {
            foreach ($this->measurements_needed as $measurement) {
                ...
                $measurement_value = \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Measurement::convert($measurement['value'], $measurement['unit'], $measurement['common_unit']);
                if ('area-surface' === $measurement_type) {
                    // get dimensions
                    $length = \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Measurement::convert($this->measurements_needed['length']['value'], $this->measurements_needed['length']['unit'], $this->measurements_needed['length']['common_unit']);
                    $width = \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Measurement::convert($this->measurements_needed['width']['value'], $this->measurements_needed['width']['unit'], $this->measurements_needed['width']['common_unit']);
                    $height = \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Measurement::convert($this->measurements_needed['height']['value'], $this->measurements_needed['height']['unit'], $this->measurements_needed['height']['common_unit']);
            ...
            // get common unit
            if ($product_measurement = \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Product::get_product_measurement($product, $settings)) {
                ...
                $measurement_needed = \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Measurement::convert($measurement_needed, $product_measurement->get_unit_common(), $settings->get_pricing_unit());
            }
    ...
    public function dynamic_pricing_adjusted_price($adjusted_price, $cart_item_key, $original_price)
    {
        ...
        if (isset($cart_item_data['pricing_item_meta_data']['_measurement_needed']) && $cart_item_data['pricing_item_meta_data']['_measurement_needed']) {
            ...
            $adjusted_price = \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Product::calculate_price($product, $cart_item_data['pricing_item_meta_data']['_measurement_needed'], $cart_item_data['pricing_item_meta_data']['_measurement_needed_unit']);
        }
    ...
    public function set_order_item_meta($item, $cart_item_key, $values)
    {
        ...
        if (isset($values['pricing_item_meta_data'])) {
            ...
            if (isset($values['pricing_item_meta_data']['_quantity']) && \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Product::pricing_calculator_inventory_enabled($item->get_product())) {
                // set the actual unit quantity (ie *2* fabrics at 3 ft each, rather than '6')
    ...
    {
        if (!$item instanceof \WDFQVendorFree\WC_Order_item_Product) {
            return;
        }
    ...
    {
        if (\WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Product::pricing_calculator_inventory_enabled($item->get_product()) && ($measurement_data = $item->get_meta('_fq_measurement_data'))) {
    ...
    public function set_product_shipping_methods($cart_item)
    {
        ...
        if ($settings->is_shipping_table_enabled()) {
            $measurement = new \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Measurement($cart_item['pricing_item_meta_data']['_measurement_needed_unit'], $cart_item['pricing_item_meta_data']['_measurement_needed']);
            $class_id = $settings->get_shipping_class_id($measurement);
            $class_id = empty($class_id) ? $product->get_shipping_class_id() : $class_id;
    ...
    public function set_product_prices($cart_item)
    {
        ...
        $product = isset($cart_item['variation_id']) && $cart_item['variation_id'] ? \wc_get_product($cart_item['product_id']) : $cart_item['data'];
        $settings = new \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Settings($product);
        if (isset($cart_item['pricing_item_meta_data']['_price']) && !\WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Product::pricing_calculator_inventory_enabled($product)) {
        ...
        } elseif (\WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Product::pricing_calculator_inventory_enabled($product)) {
            if ($settings->pricing_rules_enabled()) {
                ...
                $measurement = new \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Measurement($cart_item['pricing_item_meta_data']['_measurement_needed_unit'], $cart_item['quantity']);
    ...
    public function calculate_product_weights($cart)
    {
        ...
        foreach ($cart->cart_contents as $cart_item_key => &$values) {
            ...
            if (isset($values['pricing_item_meta_data']['_measurement_needed_unit'], $values['pricing_item_meta_data']['_measurement_needed']) && \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Product::pricing_calculated_weight_enabled($_product)) {
                $settings = new \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Settings($product);
                if ('weight' === $settings->get_calculator_type()) {
                    ...
                    $supplied_weight = new \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Measurement($values['pricing_item_meta_data']['_measurement_needed_unit'], $values['pricing_item_meta_data']['_measurement_needed']);
    ...
    public function order_again_cart_item_data($cart_item_data, $item, $order)
    {
        ...
        if (\false === \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Product::pricing_calculator_enabled($product) || !isset($item['item_meta']['_fq_measurement_data'])) {
            return $cart_item_data;
        }
        ...
        $measurement_needed = new \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Measurement($measurement_data['_measurement_needed_unit'], $measurement_data['_measurement_needed']);
    ...
    public function setup_measurement_overage_data($cart_item_data, $measurement_needed_value, $measurement_needed_value_unit, $_product, $settings = null, $order_again = \false)
    {
        ...
        $settings = null === $settings ? $settings = new \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Settings($_product) : $settings;
        ...
        if ($pricing_overage_percentage > 0) {
            ...
            $cart_item_data['pricing_item_meta_data']['_price_overage'] = \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Product::calculate_price($_product, $measurement_needed_value_overage, $measurement_needed_value_unit);
     ...
     * @param Measurement  $measurement_needed                    the total
     *                                                            measurement desired, ie 1 m, 3 sq. ft., etc
     * @param array        $measurements                          array of WC_Price_Calculator_Measurement product
     *                                                            measurements, ie 1 m or 1.5 ft, 1.5 ft.  Defaults to
     *                                                            $measurement_needed for convenience for calculators
     *                                                            with a single measurement like dimension, simple
     *                                                            area, etc.
     * @param int|float    $quantity                              contains the quantity of the item to add
     * @param int|string   $variation_id                          optional variation id
     * @param array|string $variation                             optional attribute values
     * @param array        $cart_item_data                        optional extra cart item data we want to pass into
     *                                                            the item
     *
     ...
     * @since 3.0
     */
    public function add_to_cart($product_id, $measurement_needed, $measurements = [], $quantity = 1, $variation_id = '', $variation = '', $cart_item_data = [])
    {
    ...
    public function get_item_price($item)
    {
        ...
        if (isset($item['pricing_item_meta_data']['_price']) && \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Product::pricing_calculator_inventory_enabled($item['data'])) {
    ...
    public function get_item_quantity($item)
    {
        ...
        if (isset($item['pricing_item_meta_data']['_quantity']) && \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Product::pricing_calculator_inventory_enabled($item['data'])) {
    ...
    private function get_measurement_cart_item_data($item, $cart_item_data)
    {
        ...
        // special case for calculated inventory products: the actual quantity (ie *1* item 10 feet long)
        // is held in _quantity while $item['quantity'] would be '10' in this example
        if (isset($cart_item_data['_quantity']) && \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Product::pricing_calculator_inventory_enabled($product)) {
    ...
    private function humanize_cart_item_data($item, $cart_item_data)
    {
        ...
        foreach ($calculator_measurements as $measurement) {
            ...
            if (isset($cart_item_data[$measurement_name])) {
                ...
                $label = $measurement->get_unit_label() ? \sprintf('%1$s (%2$s)', $measurement->get_label(), \__($measurement->get_unit_label(), 'flexible-quantity-measurement-price-calculator-for-woocommerce')) : \__($measurement->get_label(), 'flexible-quantity-measurement-price-calculator-for-woocommerce');
        ...
        if (isset($cart_item_data['_measurement_needed']) && $settings->is_calculator_type_derived() && ($product_measurement = \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Product::get_product_measurement($product, $settings))) {
            ...
            if (isset($cart_item_data['_measurement_needed_overage'])) {
                $overage_amount_text = \apply_filters('fq_price_calculator_overage_amount_text', $product_measurement->get_unit_label() ? \sprintf(\__('Overage %1$s (%2$s)', 'flexible-quantity-measurement-price-calculator-for-woocommerce'), $product_measurement->get_label(), \__($product_measurement->get_unit_label(), 'flexible-quantity-measurement-price-calculator-for-woocommerce')) : \sprintf(\__('Overage %s', 'flexible-quantity-measurement-price-calculator-for-woocommerce'), $product_measurement->get_label()), $item);
    ...
    public function clear_inputs_cookie($cart_item_key, $product_id)
    {
        ...
        if (\array_key_exists($cookie_name, $_COOKIE)) {
            ...
            \setcookie($cookie_name, ' ', \time() - YEAR_IN_SECONDS, '/');
        }
    }
    /**
    ...
    public function add_to_cart_item_quantity_update($cart_item_key)
    {
        ...
        $cart_item_data = \WC()->cart->cart_contents[$cart_item_key];
        $this->set_product_prices($cart_item_data);
