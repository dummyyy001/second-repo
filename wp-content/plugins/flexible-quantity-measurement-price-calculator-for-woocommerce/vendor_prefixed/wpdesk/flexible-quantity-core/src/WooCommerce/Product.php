...
class Product
{
    ...
    {
        // basic checks
        if (!$product instanceof \WC_Product || $product->is_type('grouped') || $product->is_type('variable')) {
            return \false;
        }
    ...
    public static function calculate_price($product, $measurement_needed_value, $measurement_needed_value_unit, $round = \false)
    {
        ...
        if (self::pricing_calculator_enabled($parent)) {
            $settings = new \WDFQVendorFree\WPDesk\Library\FlexibleQuantityCore\WooCommerce\Settings($parent);
            ...
            if ($settings->pricing_rules_enabled()) {
                ...
                $product->set_price($settings->get_basic_regular_price());
            }
            // calculate the price
            $price = !empty($product->get_price('edit')) ? $product->get_price('edit') : $price;
    ...
    public static function get_pricing_rules_price_html($product)
    {
        ...
        $pricing_label = $sep . ' ' . \__($settings->get_pricing_label(), 'flexible-quantity-measurement-price-calculator-for-woocommerce');
        // Get the price
        if ($price > 0) {
            // Regular price
