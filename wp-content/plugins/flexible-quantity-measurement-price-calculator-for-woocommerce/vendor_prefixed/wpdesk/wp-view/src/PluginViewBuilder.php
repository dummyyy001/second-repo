...
namespace WDFQVendorFree\WPDesk\View;
...
use WDFQVendorFree\WPDesk\View\Resolver\WPThemeResolver;
/**
...
class PluginViewBuilder
{
    ...
    public function createSimpleRenderer()
    {
        ...
        $resolver->appendResolver(new \WDFQVendorFree\WPDesk\View\Resolver\WPThemeResolver(\basename($this->plugin_dir)));
        foreach ($this->template_dirs as $dir) {
            $dir = \trailingslashit($this->plugin_dir) . \trailingslashit($dir);
            $resolver->appendResolver(new \WDFQVendorFree\WPDesk\View\Resolver\DirResolver($dir));
        }
