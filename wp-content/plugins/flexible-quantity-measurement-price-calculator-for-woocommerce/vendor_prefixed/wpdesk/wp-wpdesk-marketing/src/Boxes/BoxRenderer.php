...
namespace WDFQVendorFree\WPDesk\Library\Marketing\Boxes;

use WDFQVendorFree\WPDesk\Library\Marketing\Boxes\Abstracts\BoxInterface;
...
class BoxRenderer
{
    ...
    public function __construct(array $boxes, \WDFQVendorFree\WPDesk\View\Renderer\Renderer $renderer = null)
    {
        $this->boxes = $boxes;
        ...
        $this->bbcodes = new \WDFQVendorFree\WPDesk\Library\Marketing\Boxes\Helpers\BBCodes();
        $this->markers = new \WDFQVendorFree\WPDesk\Library\Marketing\Boxes\Helpers\Markers();
    }
     ...
     */
    public function get_box_type(array $box) : \WDFQVendorFree\WPDesk\Library\Marketing\Boxes\Abstracts\BoxInterface
    {
        switch ($box['type']) {
            case 'slider':
                return new \WDFQVendorFree\WPDesk\Library\Marketing\Boxes\BoxType\SliderBox($box, $this->renderer);
            case 'image':
                return new \WDFQVendorFree\WPDesk\Library\Marketing\Boxes\BoxType\ImageBox($box, $this->renderer);
            case 'video':
                return new \WDFQVendorFree\WPDesk\Library\Marketing\Boxes\BoxType\VideoBox($box, $this->renderer);
            case 'simple':
                return new \WDFQVendorFree\WPDesk\Library\Marketing\Boxes\BoxType\SimpleBox($box, $this->renderer);
            default:
                return new \WDFQVendorFree\WPDesk\Library\Marketing\Boxes\BoxType\UnknownBox($box, $this->renderer);
        }
    }
}
