...
use WDFQVendorFree\WPDesk\PluginBuilder\Plugin\Activateable;
use WDFQVendorFree\WPDesk\PluginBuilder\Plugin\Deactivateable;
...
trait BuilderTrait
{
     ...
     * @return SlimPlugin
     */
    private function build_plugin(\WDFQVendorFree\WPDesk_Plugin_Info $plugin_info)
    {
     ...
     * @return SlimPlugin
     */
    private function init_register_hooks(\WDFQVendorFree\WPDesk_Plugin_Info $plugin_info, \WDFQVendorFree\WPDesk\PluginBuilder\Plugin\SlimPlugin $plugin)
    {
        if ($plugin instanceof \WDFQVendorFree\WPDesk\PluginBuilder\Plugin\Activateable) {
            \register_activation_hook($plugin_info->get_plugin_file_name(), [$plugin, 'activate']);
        }
        if ($plugin instanceof \WDFQVendorFree\WPDesk\PluginBuilder\Plugin\Deactivateable) {
     ...
     * @param SlimPlugin $plugin
     */
    private function store_plugin(\WDFQVendorFree\WPDesk\PluginBuilder\Plugin\SlimPlugin $plugin)
    {
        $storageFactory = new \WDFQVendorFree\WPDesk\PluginBuilder\Storage\StorageFactory();
     ...
     * @param SlimPlugin $plugin
     */
    private function init_plugin(\WDFQVendorFree\WPDesk\PluginBuilder\Plugin\SlimPlugin $plugin)
    {
