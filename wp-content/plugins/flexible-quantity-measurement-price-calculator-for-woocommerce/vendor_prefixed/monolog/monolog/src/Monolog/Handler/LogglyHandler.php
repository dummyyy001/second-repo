...
use WDFQVendorFree\Monolog\Formatter\LogglyFormatter;
 ...
 */
class LogglyHandler extends \WDFQVendorFree\Monolog\Handler\AbstractProcessingHandler
{
     ...
     */
    public function __construct(string $token, $level = \WDFQVendorFree\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if (!\extension_loaded('curl')) {
            throw new \WDFQVendorFree\Monolog\Handler\MissingExtensionException('The curl extension is needed to use the LogglyHandler');
        }
        $this->token = $token;
    ...
    protected function send(string $data, string $endpoint) : void
    {
        ...
        \WDFQVendorFree\Monolog\Handler\Curl\Util::execute($ch, 5, \false);
    }
    protected function getDefaultFormatter() : \WDFQVendorFree\Monolog\Formatter\FormatterInterface
    {
        return new \WDFQVendorFree\Monolog\Formatter\LogglyFormatter();
    }
}
