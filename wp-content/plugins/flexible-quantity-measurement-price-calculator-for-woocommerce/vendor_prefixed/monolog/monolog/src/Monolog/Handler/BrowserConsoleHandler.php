 ...
 */
class BrowserConsoleHandler extends \WDFQVendorFree\Monolog\Handler\AbstractProcessingHandler
{
    /** @var bool */
    ...
    {
        return new \WDFQVendorFree\Monolog\Formatter\LineFormatter('[[%channel%]]{macro: autolabel} [[%level_name%]]{font-weight: bold} %message%');
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return [\WDFQVendorFree\Monolog\Logger::DEBUG => 'debug', \WDFQVendorFree\Monolog\Logger::INFO => 'info', \WDFQVendorFree\Monolog\Logger::NOTICE => 'info', \WDFQVendorFree\Monolog\Logger::WARNING => 'warn', \WDFQVendorFree\Monolog\Logger::ERROR => 'error', \WDFQVendorFree\Monolog\Logger::CRITICAL => 'error', \WDFQVendorFree\Monolog\Logger::ALERT => 'error', \WDFQVendorFree\Monolog\Logger::EMERGENCY => 'error'][$level] ?? 'log';
    }
    /**
     * @return string[]
     */
    private static function handleStyles(string $formatted) : array
    {
        ...
        foreach (\array_reverse($matches) as $match) {
            ...
            $pos = $match[0][1];
            $format = \WDFQVendorFree\Monolog\Utils::substr($format, 0, $pos) . '%c' . $match[1][0] . '%c' . \WDFQVendorFree\Monolog\Utils::substr($format, $pos + \strlen($match[0][0]));
        }
    ...
    private static function handleCustomStyles(string $style, string $string) : string
    {
        ...
        if (null === $style) {
            ...
            throw new \RuntimeException('Failed to run preg_replace_callback: ' . $pcreErrorCode . ' / ' . \WDFQVendorFree\Monolog\Utils::pcreLastErrorMessage($pcreErrorCode));
        }
        return $style;
    }
    /**
