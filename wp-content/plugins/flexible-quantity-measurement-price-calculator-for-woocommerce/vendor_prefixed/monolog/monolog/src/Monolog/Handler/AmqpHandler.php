...
use WDFQVendorFree\Monolog\Formatter\JsonFormatter;
use WDFQVendorFree\PhpAmqpLib\Message\AMQPMessage;
use WDFQVendorFree\PhpAmqpLib\Channel\AMQPChannel;
use AMQPExchange;
/**
 * @phpstan-import-type Record from \Monolog\Logger
 */
class AmqpHandler extends \WDFQVendorFree\Monolog\Handler\AbstractProcessingHandler
{
    /**
     ...
     */
    public function __construct($exchange, ?string $exchangeName = null, $level = \WDFQVendorFree\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if ($exchange instanceof \WDFQVendorFree\PhpAmqpLib\Channel\AMQPChannel) {
    ...
    private function createAmqpMessage(string $data) : \WDFQVendorFree\PhpAmqpLib\Message\AMQPMessage
    {
        $attributes = ['delivery_mode' => 2, 'content_type' => 'application/json'];
        if ($this->extraAttributes) {
            $attributes = \array_merge($attributes, $this->extraAttributes);
        }
        return new \WDFQVendorFree\PhpAmqpLib\Message\AMQPMessage($data, $attributes);
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return new \WDFQVendorFree\Monolog\Formatter\JsonFormatter(\WDFQVendorFree\Monolog\Formatter\JsonFormatter::BATCH_MODE_JSON, \false);
    }
}
