 ...
 */
class CubeHandler extends \WDFQVendorFree\Monolog\Handler\AbstractProcessingHandler
{
     ...
     */
    public function __construct(string $url, $level = \WDFQVendorFree\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
    ...
    protected function connectUdp() : void
    {
        if (!\extension_loaded('sockets')) {
            throw new \WDFQVendorFree\Monolog\Handler\MissingExtensionException('The sockets extension is required to use udp URLs with the CubeHandler');
        }
    ...
    protected function connectHttp() : void
    {
        if (!\extension_loaded('curl')) {
            throw new \WDFQVendorFree\Monolog\Handler\MissingExtensionException('The curl extension is required to use http URLs with the CubeHandler');
        }
    ...
    protected function write(array $record) : void
    {
        ...
        if ($this->scheme === 'http') {
            $this->writeHttp(\WDFQVendorFree\Monolog\Utils::jsonEncode($data));
        } else {
            $this->writeUdp(\WDFQVendorFree\Monolog\Utils::jsonEncode($data));
        }
    }
    ...
    private function writeHttp(string $data) : void
    {
        ...
        \WDFQVendorFree\Monolog\Handler\Curl\Util::execute($this->httpConnection, 5, \false);
    }
}
