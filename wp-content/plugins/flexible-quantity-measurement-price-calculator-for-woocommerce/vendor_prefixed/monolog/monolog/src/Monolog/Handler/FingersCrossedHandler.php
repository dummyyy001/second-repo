...
use WDFQVendorFree\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy;
use WDFQVendorFree\Monolog\Handler\FingersCrossed\ActivationStrategyInterface;
 ...
 */
class FingersCrossedHandler extends \WDFQVendorFree\Monolog\Handler\Handler implements \WDFQVendorFree\Monolog\Handler\ProcessableHandlerInterface, \WDFQVendorFree\Monolog\ResettableInterface, \WDFQVendorFree\Monolog\Handler\FormattableHandlerInterface
{
    ...
    public function __construct($handler, $activationStrategy = null, int $bufferSize = 0, bool $bubble = \true, bool $stopBuffering = \true, $passthruLevel = null)
    {
        if (null === $activationStrategy) {
            $activationStrategy = new \WDFQVendorFree\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy(\WDFQVendorFree\Monolog\Logger::WARNING);
        }
        // convert simple int activationStrategy to an object
        if (!$activationStrategy instanceof \WDFQVendorFree\Monolog\Handler\FingersCrossed\ActivationStrategyInterface) {
            $activationStrategy = new \WDFQVendorFree\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy($activationStrategy);
        }
        ...
        if ($passthruLevel !== null) {
            $this->passthruLevel = \WDFQVendorFree\Monolog\Logger::toMonologLevel($passthruLevel);
        }
        if (!$this->handler instanceof \WDFQVendorFree\Monolog\Handler\HandlerInterface && !\is_callable($this->handler)) {
    ...
    public function reset()
    {
        ...
        if ($this->getHandler() instanceof \WDFQVendorFree\Monolog\ResettableInterface) {
    ...
    {
        if (!$this->handler instanceof \WDFQVendorFree\Monolog\Handler\HandlerInterface) {
    ...
    public function setFormatter(\WDFQVendorFree\Monolog\Formatter\FormatterInterface $formatter) : \WDFQVendorFree\Monolog\Handler\HandlerInterface
    {
        ...
        if ($handler instanceof \WDFQVendorFree\Monolog\Handler\FormattableHandlerInterface) {
