 ...
 */
namespace WDFQVendorFree\Monolog;
 ...
 */
class Logger implements \Psr\Log\LoggerInterface, \WDFQVendorFree\Monolog\ResettableInterface
{
    /**
     ...
     */
    public function pushHandler(\WDFQVendorFree\Monolog\Handler\HandlerInterface $handler) : self
    {
     ...
     */
    public function popHandler() : \WDFQVendorFree\Monolog\Handler\HandlerInterface
    {
        if (!$this->handlers) {
     ...
     */
    public function addRecord(int $level, string $message, array $context = [], \WDFQVendorFree\Monolog\DateTimeImmutable $datetime = null) : bool
    {
        ...
        try {
            ...
            foreach ($this->handlers as $handler) {
                if (null === $record) {
                    ...
                    $record = ['message' => $message, 'context' => $context, 'level' => $level, 'level_name' => $levelName, 'channel' => $this->name, 'datetime' => $datetime ?? new \WDFQVendorFree\Monolog\DateTimeImmutable($this->microsecondTimestamps, $this->timezone), 'extra' => []];
                    try {
    ...
    {
        foreach ($this->handlers as $handler) {
            if ($handler instanceof \WDFQVendorFree\Monolog\ResettableInterface) {
                $handler->reset();
            }
        }
        foreach ($this->processors as $processor) {
            if ($processor instanceof \WDFQVendorFree\Monolog\ResettableInterface) {
                $processor->reset();
            }
        }
    }
