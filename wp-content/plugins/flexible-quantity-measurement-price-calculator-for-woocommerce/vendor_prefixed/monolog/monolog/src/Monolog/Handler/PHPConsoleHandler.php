...
use WDFQVendorFree\PhpConsole\Connector;
use WDFQVendorFree\PhpConsole\Handler as VendorPhpConsoleHandler;
use WDFQVendorFree\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \WDFQVendorFree\Monolog\Handler\AbstractProcessingHandler
{
     ...
     */
    public function __construct(array $options = [], ?\WDFQVendorFree\PhpConsole\Connector $connector = null, $level = \WDFQVendorFree\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if (!\class_exists('WDFQVendorFree\\PhpConsole\\Connector')) {
    ...
    private function initConnector(?\WDFQVendorFree\PhpConsole\Connector $connector = null) : \WDFQVendorFree\PhpConsole\Connector
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \WDFQVendorFree\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \WDFQVendorFree\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\WDFQVendorFree\PhpConsole\Helper::isRegistered()) {
            \WDFQVendorFree\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \WDFQVendorFree\PhpConsole\Handler::getInstance();
    ...
    public function getConnector() : \WDFQVendorFree\PhpConsole\Connector
    {
    ...
    {
        if ($record['level'] < \WDFQVendorFree\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record) : void
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \WDFQVendorFree\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \WDFQVendorFree\Monolog\Formatter\LineFormatter('%message%');
    }
}
