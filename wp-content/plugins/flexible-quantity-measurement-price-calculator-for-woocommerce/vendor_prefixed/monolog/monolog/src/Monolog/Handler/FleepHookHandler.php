 ...
 */
class FleepHookHandler extends \WDFQVendorFree\Monolog\Handler\SocketHandler
{
     ...
     */
    public function __construct(string $token, $level = \WDFQVendorFree\Monolog\Logger::DEBUG, bool $bubble = \true, bool $persistent = \false, float $timeout = 0.0, float $writingTimeout = 10.0, ?float $connectionTimeout = null, ?int $chunkSize = null)
    {
        if (!\extension_loaded('openssl')) {
            throw new \WDFQVendorFree\Monolog\Handler\MissingExtensionException('The OpenSSL PHP extension is required to use the FleepHookHandler');
        }
        $this->token = $token;
    ...
    {
        return new \WDFQVendorFree\Monolog\Formatter\LineFormatter(null, null, \true, \true);
    }
    /**
