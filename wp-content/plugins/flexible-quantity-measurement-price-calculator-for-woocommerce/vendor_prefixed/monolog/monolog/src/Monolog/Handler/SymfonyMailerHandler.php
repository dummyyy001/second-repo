...
use WDFQVendorFree\Symfony\Component\Mailer\MailerInterface;
use WDFQVendorFree\Symfony\Component\Mailer\Transport\TransportInterface;
use WDFQVendorFree\Symfony\Component\Mime\Email;
/**
 ...
 */
class SymfonyMailerHandler extends \WDFQVendorFree\Monolog\Handler\MailHandler
{
     ...
     */
    public function __construct($mailer, $email, $level = \WDFQVendorFree\Monolog\Logger::ERROR, bool $bubble = \true)
    {
     ...
     */
    protected function buildMessage(string $content, array $records) : \WDFQVendorFree\Symfony\Component\Mime\Email
    {
        $message = null;
        if ($this->emailTemplate instanceof \WDFQVendorFree\Symfony\Component\Mime\Email) {
            $message = clone $this->emailTemplate;
        } elseif (\is_callable($this->emailTemplate)) {
            $message = ($this->emailTemplate)($content, $records);
        }
        if (!$message instanceof \WDFQVendorFree\Symfony\Component\Mime\Email) {
            $record = \reset($records);
            throw new \InvalidArgumentException('Could not resolve message as instance of Email or a callable returning it' . ($record ? \WDFQVendorFree\Monolog\Utils::getRecordMessageForException($record) : ''));
        }
        if ($records) {
