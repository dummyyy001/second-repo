...
class Kadence_Blocks_Pro_Filter_Buttons_Block extends Kadence_Blocks_Query_Children_Block {
	...
	public function build_css( $attributes, $css, $unique_id, $unique_style_id ) {
		...
		$css->set_media_state( 'desktop' );
		$css->set_selector( '.wp-block-kadence-query-filter-buttons' . $unique_id . ' .buttons-options .kb-button' );
		...
		// Hover Style.
		$css->set_selector( '.wp-block-kadence-query-filter-buttons' . $unique_id . ' .buttons-options .kb-button:hover' );
		...
		if ( isset( $attributes['displayHoverShadowButton'] ) && true === $attributes['displayHoverShadowButton'] ) {
			if ( ( 'gradient' === $bg_type || 'gradient' === $bg_hover_type ) && isset( $attributes['shadowHoverButton'][0]['inset'] ) && true === $attributes['shadowHoverButton'][0]['inset'] ) {
				...
				$css->set_selector( '.wp-block-kadence-query-filter-buttons' . $unique_id . ' .buttons-options .kb-button:hover::before' );
			}
		...
		// Active Styles.
		$css->set_selector( '.wp-block-kadence-query-filter-buttons' . $unique_id . ' .buttons-options .kb-button.pressed' );
