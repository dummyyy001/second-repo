...
class Kadence_Blocks_Dynamic_Content_Controller extends WP_REST_Controller {
	 ...
	 * @return array.
	 */
	public function get_options_from_fields( $fields, $types, $is_option_page, $prefix = 'acf_meta', $suffix = '', $outer_label = 'Sub Fields', $field_id_key = 'name', $field_label_key = 'label', $field_type_key = 'type', $field_prefix = '' ) {
		$options = array();
		...
		foreach ( $fields as $field ) {
			...
			// Special case for acf group fields. (works for meta box groups in theory as weel, but they don't run through here currently)
			if ( $field[ $field_type_key ] == 'group' ) {
				if ( isset( $field['sub_fields'] ) && $field['sub_fields'] ) {
					$options = array_merge( $options, $this->get_options_from_fields( $field['sub_fields'], $types, $is_option_page, $prefix, $suffix, $field['label'], $field_id_key, $field_label_key, $field_type_key, $field['name'] . '_' . $field_prefix ) );
				}
				continue;
			}
				...
				if ( ! empty( $meta_boxes ) && is_array( $meta_boxes ) ) {
					...
					foreach ( $meta_boxes as $mb_key => $mb_group ) {
						...
						if ( ! empty( $mb_group->meta_box ) && is_array( $mb_group->meta_box ) && ! empty( $mb_group->meta_box['fields'] ) && is_array( $mb_group->meta_box['fields'] ) ) {
							...
							$options = array_merge( $options, $this->get_options_from_fields( $fields, $types, false, $prefix, $suffix, $outer_label, $field_id_key, $field_label_key ) );
						}
					}
				}
			}
